...
!
! Last changed by Vadym Lepetyuk, September 17, 2004
...
csi%bmin=-3.5; csi%bmax=5.0  !Boundaries for b
csi%pkmin=0.0; csi%pkmax=2.0  !Boundaries for pk
CALL initgrid(csi)
...
DO x=1.0,-0.005,-0.005
	...
	IF (csi.pkmax<xssx(i__pk)) THEN
		WRITE(unit1,*) 'WARNING: In order to calculate compensation for regime change'
		WRITE(unit1,*) 'WARNING: we keep steady state for x=1 inside the grid for current x'
		csi.pkmax=csix.pkmax
	END IF
...
csi%bmin=-3.5; csi%bmax=5.0  !Boundaries for b
csi%pkmin=0.0; csi%pkmax=2.5  !Boundaries for pk
x=1.0; y=0.0 !Initialize x and y
CALL initcsi(csi) !Compute initial guess for csi
CALL solvecsi(csi) !Compute csi
...
csix%bmin=csi%bmin; csix%bmax=csi%bmax
csix%pkmin=csi%pkmin; csix%pkmax=csi%pkmax
CALL reinitgrid(csix) !Recompute Chebyshev interpolation nodes
csix%var=csi%var !Keep csi values, calculated above
csix%coef=csi%coef !Keep Chebyshev coefficients as well
WRITE(unit0,*) '---------------- budgetrules: Loop over y ----------------'
WRITE(unit0,*) '(y,utility,wedgeG,wedgepk,compensation)'
...
DO y=0.0,0.06,0.005
	...
	IF (csi.pkmax<xssx(i__pk)) THEN
		WRITE(unit1,*) 'WARNING: In order to calculate compensation for regime change'
		WRITE(unit1,*) 'WARNING: we keep steady state for y=0 inside the grid for current y'
		csi.pkmax=csix.pkmax
	END IF
	...
	CALL efficiency(xss,wedgeG,wedgepk) 	!Asymptotic Efficiency
	CALL compensation(csi,xssx,compens) 	!Compensation for regime change
	WRITE(unit0,'(5F12.5)') y,u,wedgeG,wedgepk,compens
